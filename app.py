...
def category(category):
    response.content_type = 'application/json; charset=utf-8'
    db = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    sql = "SELECT category_id, sub_category_name FROM category_topics ct WHERE ct.category_id = '" + category + "' GROUP BY category_id, sub_category_name;"
    db.execute(sql)
    rows = db.fetchall()
    ...
    for row in rows:
        ...
        sql = "SELECT * FROM category_topics ct WHERE ct.category_id = '" + category + "' and ct.sub_category_name = '" + row['sub_category_name'] + "';"
        ...
        data[row['category_id']].append(sub_topic)
